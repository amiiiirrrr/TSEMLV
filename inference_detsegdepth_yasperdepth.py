...
# from src.segment_frames_segformer import SegmentMMSegmentation
...
class Run:
            ...
            if len(object_boxes) > 0:
                ...
                if len(largest_tumor) > 0:
                    ...
                    if exist_SI and (len(largest_tumor) > 0):
                        ...
                        segmentation_SI_tumor = self.create_segmentation_SI_Tumor(img_original, visual, mask_SI_tumor)
                        # img_cropped = self.remove_black_pixels(img_original)
                        ...
                        # depth_raw = self.idepth_to_depth(idepth)

                        # print('name_image', name_image)
                        normalized_depth = np.load(f'data/yasper_depth_model/depthmaps_npy/{name_image}.npy')
                        # print('normalized_depth', normalized_depth)
                        normalized_depth = self.idepth_to_depth(normalized_depth)
                        # normalized_depth = self.normalize_depth(normalized_depth)
                        plt.imsave(os.path.join(self.path_save, 'prediction_depth.png'), normalized_depth)
                        depth_map_visualize = self.write_depth_yasper(normalized_depth)
                        plt.imsave(os.path.join(self.path_save, 'depth_map_visualize.png'), depth_map_visualize)
    ...
    def remove_black_pixels(self, image):
        ...
        # cv2.imwrite(os.path.join(self.path_save, 'thresholded.png'), thresholded)
    ...
    def idepth_to_depth(self, idepth):
        depth = 1 / (idepth)
        # depth[depth >= 5 * self.FocalLength] = np.inf
        # depth[depth >= 5 * self.FocalLength] = 5 * self.FocalLength
        return depth
    ...
    def calculate_average_image_RGB(self, img):
        # Calculate the sum of the pixel values for each color channel
    ...
    def write_depth_yasper(self, depth):
